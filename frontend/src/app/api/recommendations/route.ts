...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(
      `${getBackendBaseUrl()}/movies/userRecommendations?userId=${encodeURIComponent(userId)}`,
      {
        cache: "no-store"
      }
    );
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: "Recommendation request failed.", details: payload },
    ...
    return NextResponse.json(
      {
        message: "Unable to connect to the backend recommendation endpoint.",
